...
from create_db import get_words, user_list, add_user, add_user_word, get_user_words, delete_user_word
...
def create_cards(message):
    ...
    buttons = []
    others = []
    get_word = random.sample((get_words()+get_user_words(cid)), 4)
    word = get_word[0]
    target_word = word[0]
    translate = word[1]
...
def delete_word(message):
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        cid = message.chat.id
        userStep[cid] = 2
        bot.send_message(cid, "Введите слово, которое хотите удалить:")
        bot.set_state(message.from_user.id, MyStates.target_word, message.chat.id)
...
def add_word(message):
    cid = message.chat.id
    userStep[cid] = 1
    bot.send_message(cid, "Введи слово на английском и через пробел его перевод:")
...
def message_reply(message):
    ...
    cid = message.chat.id
    all_words = []
    for i in (get_words() + get_user_words(cid)):
    ...
    if userStep[cid] == 0:
        ...
        user_words = []
        for words in get_user_words(cid):
            user_words.append(words)
        if text not in get_user_words(cid):
